...
/**
 * Created by rbhunia on 11/30/2016.
 */
...
def issueOnEvents(Issue issue) {

    logger.info("*******************************************************************************")
    logger.info("Start executing auto assignment logic for issue ${issue.getKey()} from listener")
    ...
    logger.info("Completed executing auto assignment logic for issue ${issue.getKey()}  from listener")
    logger.info("************************************************************************************")
...
def setAssignee(Issue issue, autoAssigneeConfig) {
    ...
    //updateIssueWhenAssigneeChanged(issue)
}
...
def updateIssue(issue) {
    ...
    issueManager.updateIssue(user, (MutableIssue) issue, EventDispatchOption.ISSUE_UPDATED, true)
}
